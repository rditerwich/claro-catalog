...
import claro.catalog.manager.businesslogic.Catalog;
...
public class DefaultContent {
  ...
  def createInitialCatalog(createInitalData : => Any) : Catalog = transaction { em =>
	...
	public claro.jpa.catalog.Catalog getOrCreateCatalog(String name) {
		claro.jpa.catalog.Catalog result = findSingle("SELECT c FROM Catalog c WHERE c.name = :name", Tuple.create("name", name));
		if (result == null) {
			result = new claro.jpa.catalog.Catalog();
			result.setName(name);
	...
	private claro.jpa.shop.Shop getOrCreateShop(String name, claro.jpa.catalog.Catalog catalog) {
		claro.jpa.shop.Shop result = findShop(name);

		if (result == null) {
			result = new claro.jpa.shop.Shop();
			result.setName(name);
	...
	private claro.jpa.shop.Shop findShop(String name) {
