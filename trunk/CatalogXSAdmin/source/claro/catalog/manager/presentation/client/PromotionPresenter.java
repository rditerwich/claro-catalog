...
import claro.catalog.manager.presentation.client.binding.HasValueBinding;
...
import claro.catalog.manager.presentation.client.query.ProductShopQuery;
...
import claro.catalog.manager.presentation.client.shop.Promotion;
import claro.catalog.manager.presentation.client.shop.Shop;
import claro.catalog.manager.presentation.client.shop.VolumeDiscountPromotion;
import claro.catalog.manager.presentation.client.shop.VolumeDiscountPromotionBinding;

import claro.catalog.manager.presentation.client.PromotionView.PromoView;
...
public class PromotionPresenter implements Presenter<PromotionView> {
	...
	public PromotionPresenter() {
		...
		view = new PromotionView(new SuggestOracle() {
			...
			public void requestSuggestions(final Request request,
				...
				CatalogServiceAsync.findByStringValueShopProducts(0, request
						.getLimit(), query, new AsyncCallback<List<Product>>() {
					@Override
		...
		view.getEditView().productFilterSelectionHandlers()
				.addSelectionHandler(new SelectionHandler<Suggestion>() {
					@Override
					public void onSelection(SelectionEvent<Suggestion> event) {
						final ProductSuggestion ps = (ProductSuggestion) event
								.getSelectedItem();
		...
		// we need to bind product() otherwise it will crash, because the above
		// usage
		// of bind will create an empty binding and when bindings are
		// propagated, it
		// will crash because in product() the bindings are empty, this is a
		// bug.
		HasTextBinding.bind(new HasText() {
			...
			@Override
			public Product convertFrom(String data) {
				return this.product;
			}

			@Override
			public String convertTo(Product data) {
				product = data;
				return null;
			}
		});
		HasValueBinding.bind(view.getEditView().getStartDate(),
				editPromoBinding.startDate(),
				new BindingConverter<Date, Date>() {
					@SuppressWarnings("deprecation")
					@Override
					public Date convertFrom(Date data) {
						// here we need to set the end date....
						final Date endDate = editPromoBinding.endDate()
								.getData() == null ? new Date()
								: (Date) editPromoBinding.endDate().getData();
					...
					@Override
					public Date convertTo(Date data) {
						return data;
					}
				});
		HasTextBinding.bind(view.getEditView().getEndDate(), editPromoBinding
				.endDate(), new BindingConverter<Date, String>() {
			private Date date;

			@Override
			public Date convertFrom(String data) {
				return date;
			}

			@Override
			public String convertTo(Date data) {
				this.date = data;
				return data == null ? "" : DateTimeFormat.getMediumDateFormat()
						.format(data);
			}
		});
		TextBoxBaseBinding.bind(view.getEditView().getPrice(), editPromoBinding
				.price(), BindingConverters.DOUBLE_CONVERTER);
		TextBoxBaseBinding.bind(view.getEditView().getVolumeDiscount(),
				editPromoBinding.volumeDiscount(),
				BindingConverters.INTEGER_CONVERTER);
		view.getEditView().saveClickHandlers().addClickHandler(
				new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {
						final VolumeDiscountPromotion pnew = (VolumeDiscountPromotion) editPromoBinding
								.getData();

						if (pnew.getProduct() == null)
							return;
						ShopServiceAsync.updateVolumeDiscountPromotion(
								(VolumeDiscountPromotion) orgPromo, pnew,
								new AsyncCallback<VolumeDiscountPromotion>() {
									@Override
									public void onFailure(Throwable caught) {
									...
									public void onSuccess(
											VolumeDiscountPromotion result) {
		...
		view.setDeleteHandler(new DeleteHandler<PromoView>() {
			...
			public void onDelete(final PromoView promoView) {
				...
				ShopServiceAsync.updatePromotion(delPromo, null,
						new AsyncCallback<Promotion>() {
							@Override
							public void onFailure(Throwable caught) {
								// TODO Auto-generated method stub
							}
	...
	public void show() {
		if (activeShop == null) {
			activeShop = CatalogCache.get().getCurrentShop();
		}
		if (promotions == null) {
			...
			for (int i = promoViews.size(); i < promotions.size(); i++) {
				...
				pv.editClickHandlers().addClickHandler(new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {
						final VolumeDiscountPromotion p = (VolumeDiscountPromotion) CatalogCache
								.get().getPromotion(pv.getId());

						orgPromo = p;
						editPromoBinding.setData(p.clone(new HashMap()));
						view.getEditView().setName(
								Util.productToString(p.getProduct(),
										currentLanguage));
					}
				});
				promoViews.add(pv);
	...
	private void showPromo(VolumeDiscountPromotion p, PromoView pv) {
		...
		pv.setStartDate(DateTimeFormat.getMediumDateFormat().format(
				p.getStartDate()));
		pv.setEndDate(DateTimeFormat.getMediumDateFormat().format(
				p.getEndDate()));
	...
	private void loadPromotions() {
		...
		ShopServiceAsync.findActualPromotions(fromIndex, pageSize, filter,
				new AsyncCallback<List<Promotion>>() {
					@Override
					public void onFailure(Throwable caught) {
						// TODO Auto-generated method stub
					...
					public void onSuccess(List<Promotion> result) {
						...
						promotions = result == null ? new ArrayList<Promotion>(
								1) : result;
