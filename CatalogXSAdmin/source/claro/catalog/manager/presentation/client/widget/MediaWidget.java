...
import claro.catalog.manager.presentation.client.binding.PropertyBinding;
...
public class MediaWidget extends Composite {
					...
					if (pvb != null) {
						if (((PropertyValue) pvb.getData()).getId() == null) {
							...
							if (idS != null && !"".equals(idS.trim())) {
								((PropertyValue) pvb.getData()).setId(Long
										.valueOf(idS.trim()));
							}
						}
						up.hide();
						CatalogCache.get().getPropertyValue(
								(Long) pvb.id().getData(),
									...
									public void onSuccess(PropertyValue result) {
										if (result != null) {
											if (result.getItem() == null) {
												result.setItem((Item) pvb
														.item().getData());
											}
											if (result.getProperty() == null) {
												result
														.setProperty((Property) pvb
																.property()
																.getData());
											}
											pvb.setData(result);
											StatusMessage.get().show(
													i18n.fileUploaded(result
															.getStringValue()));
										}
									}
								});
	...
	public Binding bind(final PropertyValueBinding pvb) {
		...
		HasTextBinding.<Long> bind(itemId, pvb.item().id(),
				BindingConverters.LONG_CONVERTER);
		HasTextBinding.<Long> bind(propertyId, pvb.property().id(),
				BindingConverters.LONG_CONVERTER);
		HasTextBinding.<Long> bind(pvId, pvb.id(),
				BindingConverters.LONG_CONVERTER);
		HasTextBinding.bind(language, pvb.language(),
	...
	public void show(String idS, String text, String filename) {
		final Long id = Long.valueOf(idS == null || "".equals(idS.trim()) ? "0"
				: idS.trim());
	...
	private <T> T getData(PropertyBinding<T> data, T dv) {
		return (T) (data == null || data.getData() == null ? dv : data
				.getData());
	}
}
